"""
Geração sintética das classes DD e NDD via API Groq (qwen/qwen3.8-27b) para o corpus DDSD.

Espelha o fluxo do mistral_generate.py, mas grava em project/groq/ e usa a API Groq.
...
Saída (formato final do usuário: apenas text e label):
  project/groq/dd.jsonl  -> {"text": ..., "label": "DD"}
  project/groq/ndd.jsonl -> {"text": ..., "label": "NDD"}

Uso:
  cd project
  uv run python src/dataset/groq_generate.py --target 1000 --seed-ndd opencode/dd.jsonl
...
class GroqClient:
    def __init__(self, model: str | None = None):
        ...
        if not api_key:
            raise RuntimeError("GROQ_API_KEY não definida (cheque project/.env)")
...
def main() -> None:
    ...
    BATCH = 8
    RATE_SLEEP = 12
    print(f"Groq {client.model} | alvo DD e NDD: {args.target}")
