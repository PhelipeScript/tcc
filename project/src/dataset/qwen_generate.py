"""
Geração sintética das classes DD e NDD via LLM local (qwen/qwen3.6-35b-a3b) para o corpus DDSD.
...
Saída (formato final do usuário: apenas text e label):
  project/qwen/dd.jsonl  -> {"text": ..., "label": "DD"}
  project/qwen/ndd.jsonl -> {"text": ..., "label": "NDD"}

Uso:
  cd project
  uv run python src/dataset/qwen_generate.py --target 1000 --seed-ndd opencode/dd.jsonl

Pré-requisito: endpoint remoto ativo com o modelo carregado.
Modelo padrão: qwen/qwen3.6-35b-a3b (configurável via QWEN_MODEL e QWEN_BASE_URL em project/.env).
...
class QwenClient:
    ...
    def __init__(self, model: str | None = None, base_url: str | None = None):
        ...
        self.base_url = (base_url or os.environ.get(
            "QWEN_BASE_URL", "http://100.82.82.77:1234/api/v1/chat")).rstrip("/")
        self.model = model or os.environ.get("QWEN_MODEL", "qwen/qwen3.6-35b-a3b")
    ...
    def generate(self, prompt: str, temperature: float) -> str:
        ...
        # Endpoint nativo: usa "input" (não "messages") e "reasoning": desativado ("off")
        # para evitar o raciocínio lento do modelo de raciocínio.
        payload = {
            "model": self.model,
            "reasoning": "off",
            "input": prompt,
        ...
        if resp.status_code != 200:
            raise RuntimeError(f"Qwen {resp.status_code}: {resp.text[:300]}")
        data = resp.json()
        outputs = data.get("output")
        if not isinstance(outputs, list) or not outputs:
            raise ValueError("Qwen retornou output vazio")
        content = ""
        for item in outputs:
            if isinstance(item, dict) and item.get("content"):
                ...
                break
        if not content:
            raise ValueError("Qwen retornou content vazio")
        return content
...
def main() -> None:
    ...
    BATCH = 8
    RATE_SLEEP = 12
    print(f"Qwen {client.model} | alvo DD e NDD: {args.target}")
