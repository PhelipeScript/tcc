"""
Geração sintética das classes DD e NDD via API NVIDIA build (moonshotai/kimi-k3) para o corpus DDSD.
...
Saída (formato final do usuário: apenas text e label):
  project/nvidia/dd.jsonl  -> {"text": ..., "label": "DD"}
  project/nvidia/ndd.jsonl -> {"text": ..., "label": "NDD"}

Uso:
  cd project
  uv run python src/dataset/nvidia_generate.py --target 5000 --seed-ndd opencode/dd.jsonl
...
class NVIDIAINCClient:
    """Cliente da API NVIDIA build (build.nvidia.com), compatível com OpenAI chat/completions via HTTP."""
    ...
    def __init__(self, model: str | None = None):
        ...
        if not api_key:
            raise RuntimeError("NVIDIA_API_KEY não definida (cheque project/.env)")
        self.api_key = api_key
        self.model = model or os.environ.get("NVIDIA_MODEL", "moonshotai/kimi-k3")
...
def main() -> None:
    ...
    BATCH = 8
    RATE_SLEEP = 12
    print(f"NVIDIA {client.model} | alvo DD e NDD: {args.target}")
