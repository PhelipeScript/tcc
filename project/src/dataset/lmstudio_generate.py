"""
Geração sintética das classes DD e NDD via LM Studio (modelo local via HTTP) para o corpus DDSD.
...
Saída (formato final do usuário: apenas text e label):
  project/lmstudio/dd.jsonl  -> {"text": ..., "label": "DD"}
  project/lmstudio/ndd.jsonl -> {"text": ..., "label": "NDD"}

Uso:
  cd project
  uv run python src/dataset/lmstudio_generate.py --target 5000 --seed-ndd opencode/dd.jsonl

Pré-requisito: LM Studio Local Server ativo com um modelo carregado.
Modelo padrão: google/gemma-4-e4b (configurável via LMSTUDIO_MODEL em project/.env).
...
class LMStudioClient:
    """Cliente do LM Studio Local Server (OpenAI-compatible via HTTP), sem necessidade de chave de API."""
    ...
    def __init__(self, model: str | None = None):
        ...
        self.model = model or os.environ.get("LMSTUDIO_MODEL", "google/gemma-4-e4b")

    @retry(wait=wait_exponential(multiplier=2, min=2, max=60), stop=stop_after_attempt(8))
    def generate(self, prompt: str, temperature: float) -> str:
        ...
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": "Você gera comandos de voz em português brasileiro para computador pessoal. Responda APENAS com um array JSON de strings, sem explicação e sem texto de raciocínio."},
                {"role": "user", "content": prompt},
            ],
            "temperature": temperature,
            "max_tokens": 1024,
        }
        resp = requests.post(self.API_URL, headers=headers, json=payload, timeout=600)
        if resp.status_code != 200:
            raise RuntimeError(f"LMStudio {resp.status_code}: {resp.text[:300]}")
        data = resp.json()
        content = data["choices"][0]["message"].get("content")
        if not content or not content.strip():
            raise ValueError("LMStudio retornou content vazio")
        return content
...
def main() -> None:
    ...
    BATCH = 8
    RATE_SLEEP = 12
    print(f"LMStudio {client.model} | alvo DD e NDD: {args.target}")
