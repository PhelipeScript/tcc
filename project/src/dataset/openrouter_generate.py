"""
Geração sintética das classes DD e NDD via API OpenRouter (openrouter/free) para o corpus DDSD.
...
Saída (formato final do usuário: apenas text e label):
  project/openrouter/dd.jsonl  -> {"text": ..., "label": "DD"}
  project/openrouter/ndd.jsonl -> {"text": ..., "label": "NDD"}

Uso:
  cd project
  uv run python src/dataset/openrouter_generate.py --target 5000 --seed-ndd opencode/dd.jsonl
...
class OpenRouterClient:
    """Cliente da API OpenRouter (build via /free), compatível com OpenAI chat/completions via HTTP."""
    ...
    def __init__(self, model: str | None = None):
        ...
        if not api_key:
            raise RuntimeError("OPENROUTER_API_KEY não definida (cheque project/.env)")
    ...
    def generate(self, prompt: str, temperature: float) -> str:
        ...
        if not content or not content.strip():
            raise ValueError("OpenRouter retornou content vazio (modelo de raciocínio/moderação)")
        return content
...
def main() -> None:
    ...
    BATCH = 8
    RATE_SLEEP = 12
    print(f"OpenRouter {client.model} | alvo DD e NDD: {args.target}")
